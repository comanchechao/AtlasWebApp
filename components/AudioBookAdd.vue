    ...
    >
      ...
      >
        <div class="flex items-end flex-col space-y-3 order-1 lg:-order-none">
          <label class="text-xl text-mainBlue" for="title">عنوان کتاب</label>
          <InputText
            id="title"
            v-model="title"
...
const uploadVideo = async function (event) {
  ...
  await $fetch("http://localhost:3333/audio-books/management/addbook", {
    method: "POST",
    credentials: "include",
...
const uploadImage = async function (event) {
  ...
  await $fetch("http://localhost:3333/audio-books/management/bookimage", {
    method: "POST",
    credentials: "include",
    withCredentials: true,
    body: formData,
